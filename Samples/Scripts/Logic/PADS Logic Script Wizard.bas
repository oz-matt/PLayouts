''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' Basic Report Wizard for PADS Logic                                             '
' Copyright (C) 2003 Mentor Graphics Corp.                                       '
'                                                                                '
' Basic Script Version 1.02                                                      '
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
'
'HKEY_CURRENT_USER\Software\VB and VBA Program Settings\PowerLogic 3.0 Custom Report Wizard v1.02

Option Explicit

Const WizardTitle	= "Basic Script Wizard"
Const AppName		= "PowerLogic"
Const WizName		= AppName & " 3.0 Custom Report Wizard v1.02"
Const AppId			= 1	' 1 for PADS Logic; 2 for PADS Layout
...
Const contColl = "ActiveDocument.Sheets"
Const contName = Array("Sheet", "sheet")
Const contNameProp = "aSheet.Name"
Const contObjs = Array("ActiveDocument", "aSheet")
...
'			Name0,		VarName1,	Collection2,		 SortColl3,				ObjType4				Attr5	Sht6	Index7, SecObjs8, NameProp9
Const primData = Array( _
	Array("Part",		"part",		"xxx.Components",	"xxx.Components",		plogObjectTypeComponent,True,	True,	True,	True,	"xxx.Name"), _
	Array("Gate",		"aGate",	"xxx.Gates",		"xxx.Gates",			plogObjectTypeGate,		False,	True,	True,	True,	"xxx.Name"), _
	Array("Net",		"aNet",		"xxx.Nets",			"xxx.Nets",				plogObjectTypeNet,		True,	True,	True,	True,	"xxx.Name"), _
	Array("Pin", 		"aPin",		"xxx.Pins",			"GetSortedPins(xxx)",	plogObjectTypePin,		False,	True,	True,	False,	"xxx.Name"), _
	Array("Part Type", 	"pkg",		"xxx.PartTypes",	"xxx.PartTypes",		plogObjectTypePartType,	False,	True,	True,	True,	"xxx.Name") _
)
...
Const primObjProps = Array( _
	Array( _
		...
		Array("PCB Decal", 					"", 					10, Simple,		Lf,	"xxx.PCBDecal",											1), _
		Array("Logic Family", 				"Logic", 		 		 5, Simple,		Cn,	"xxx.PartTypeLogic",									1), _
		Array("Pin Count", 					"Pins", 		 		 5, Simple,		Rt,	"xxx.Pins.Count",										1), _
		Array("Unused Pin Count", 			"Unused Pins", 			11, Simple,		Rt,	"GetUnusedPins(xxx).Count",								1), _
		Array("Signal Pin Count", 			"Sig.Pins", 			 8, Simple,		Rt,	"GetSignalPins(xxx).Count",								1), _
		Array("Gate Count", 				"Gates", 				 5,	Simple,		Rt,	"xxx.Gates.Count + xxx.UnusedGates.Count",				1), _
		Array("Unused Gate Count",			"Unused Gates", 		12,	Simple,		Rt,	"xxx.UnusedGates.Count",								1), _
		Array("Value",						"", 		  			 8,	Attr,		Lf,	"AttrVal(xxx, ""Value"")",								1), _
		Array("Tolerance",					"", 			 		 9,	Attr,		Lf,	"AttrVal(xxx, ""Tolerance"")",							1), _
		Array("Sheet Count", 				"Sheets", 		 		 6,	Simple,		Rt,	"xxx.Sheets.Count",										1), _
		Array("Connected Pin Count",		"Con.Pins",		 		 6,	Simple,		Rt,	"GetConPins(xxx).Count",								1), _
		Array("Unconnected Pin Count",		"Uncon.Pins",	 		 6,	Simple,		Rt,	"GetUnconPins(xxx).Count",								1)  _
	), _
	Array( _
		Array("Name", 						"", 					 8, Simple,		Lf,	"xxx.Name",												1), _
		Array("Number", 					"", 					 6, Simple,		Rt,	"xxx.Number",											1), _
		Array("Swap Class", 				"", 					10, Simple,		Rt,	"xxx.SwapClass",										1), _
		Array("Sheet", 						"", 					16, Simple,		Lf,	"xxx.Sheet",											1), _
		Array("Position X", 				"", 					12, Simple,		Rt,	"xxx.PositionX",										1), _
		Array("Position Y", 				"", 					12, Simple,		Rt,	"xxx.PositionY",										1), _
		Array("Connector", 					"", 					 9, Simple,		Cn,	"Format(xxx.IsConnector, ""Yes/No"")",					1), _
		Array("Mirror X", 					"", 					 8, Simple,		Cn,	"Format(xxx.ReflectedX, ""Yes/No"")",					1), _
		Array("Mirror Y", 					"", 					 8, Simple,		Cn,	"Format(xxx.ReflectedY, ""Yes/No"")",					1), _
		Array("Rotated 90", 				"", 					10, Simple,		Cn,	"Format(xxx.Rotated90, ""Yes/No"")",					1), _
		Array("Pin Count", 					"Pins",					 5, Simple,		Rt,	"xxx.Pins.Count",										1), _
		Array("Connected Pin Count",		"Con.Pins",				 6,	Simple,		Rt,	"GetConPins(xxx).Count",								1), _
		Array("Unconnected Pin Count",		"Uncon.Pins",			 6,	Simple,		Rt,	"GetUnconPins(xxx).Count",								1)  _
	), _
	Array( _
		Array("Name", 						"", 					 8, Simple,		Lf,	"xxx.Name",												1), _
		Array("Width", 						"", 					 5, Simple,		Rt,	"xxx.Width",											1), _
		Array("Pin Count", 					"", 					 9, Simple,		Rt,	"xxx.Pins.Count",										1), _
		Array("Gate Count", 				"", 					10, Simple,		Rt,	"GetNetGates(xxx).Count",								1), _
		Array("Sheet Count", 				"", 					11,	Simple,		Rt,	"xxx.Sheets.Count",										1)  _
	), _
	Array( _
		Array("Name", 						"", 					 8, Simple,		Lf, "xxx.Name",												1), _
		Array("Signal", 					"", 					10, Simple,		Lf, "ObjName(xxx.Net)",										1), _
		Array("Number", 					"", 					 6, Simple,		Rt, "xxx.Number",											1), _
		Array("Swap Class", 				"", 					10, Simple,		Rt, "xxx.SwapClass",										1), _
		Array("Electrical Type", 			"", 					15, Simple,		Lf, "PinType(xxx)",											1), _
		Array("Gate", 						"", 					 8, Simple,		Lf, "ObjName(xxx.Gate)",									1), _
		Array("Sheet", 						"", 					16, Simple,		Lf, "PinSheet(xxx)",										1), _
		Array("Position X", 				"", 					12, Simple,		Rt, "PinPosX(xxx)",											1), _
		Array("Position Y", 				"", 					12, Simple,		Rt, "PinPosY(xxx)",											1), _
		Array("Function Name", 				"", 					13, Simple,		Lf, "xxx.FunctionName",										1)  _
	), _
	Array( _
		Array("Name", 						"", 					 8, Simple,		Lf,	"xxx.Name",												1), _
		Array("Logic Family", 				"Logic", 				 5, Simple,		Cn,	"xxx.Logic",											1), _
		Array("Part Count", 				"Part Count",	 		10, Simple,		Rt,	"xxx.Components.Count",									1)  _
	) _
)

'Array of object lists
'		List Name ID,						List Header,   		   NumPerLine, OutType,VarName,	Collection,						Sorted Collection,			Filtered By sheet/ao Collection,	Sorted Filtered By Sheet/AssOpt Collection, Out																										Help String
Const primObjLists = Array( _
	Array( _
		Array("Part Pins",					"Part Pins:", 			 		5,	Simple,	"aPin",	"xxx.Pins",						"GetSortedPins(xxx)",		"GetPinsBySheet(xxx, cont)",		"GetPinsBySheet(xxx, cont, True)",		"yyy.Name\Pin Type:PinTypeShort(yyy)\Pin Signal:ObjName(yyy.Net)\Gate:PinGate(yyy)\Sheet:PinSheet(yyy)",	"List of all part pins (with optional type, signal, gate, and location info)"), _
		Array("Connected Pins",				"Connected Part Pins:", 		5,	Simple,	"aPin",	"GetConPins(xxx)",				"GetConPins(xxx, True)",	"GetConPinsBySheet(xxx, cont)",		"GetConPinsBySheet(xxx, cont, True)",	"yyy.Name\Pin Type:PinTypeShort(yyy)\Pin Signal:yyy.Net\Gate:PinGate(yyy)\Sheet:PinSheet(yyy)",				"List of only pins connected to a net (with optional type, signal, gate, and location info)"), _
		Array("Unconnected Pins",			"Unconnected Part Pins:", 		5,	Simple,	"aPin",	"GetUnconPins(xxx)",			"GetUnconPins(xxx, True)",	"GetUnconPinsBySheet(xxx, cont)", 	"GetUnconPinsBySheet(xxx, cont, True)",	"yyy.Name\Pin Type:PinTypeShort(yyy)\Gate:PinGate(yyy)\Sheet:PinSheet(yyy)",								"List of only pins not connected to a net (with optional type, gate, and location info)"), _
		Array("Used Pins", 					"Used Part Pins:", 				5,	Simple,	"aPin",	"GetUsedPins(xxx)",				"GetUsedPins(xxx, True)", 	"GetUsedPinsBySheet(xxx, cont)",	"GetUsedPinsBySheet(xxx, cont, True)",	"yyy.Name\Pin Type:PinTypeShort(yyy)\Pin Signal:ObjName(yyy.Net)\Gate:PinGate(yyy)\Sheet:PinSheet(yyy)",	"List of pins belonging to used gates or pins connected to a net (with optional type and signal)"), _
		Array("Unused Pins",	 			"Unused Part Pins:", 			5,	Simple,	"aPin",	"GetUnusedPins(xxx)",			"GetUnusedPins(xxx, True)",	"",									"",										"yyy.Name\Pin Type:PinTypeShort(yyy)",																		"List of unconnected pins not belonging to used gate"), _
		Array("Signal Pins",				"Signal Part Pins:", 			5,	Simple,	"aPin",	"GetSignalPins(xxx)",			"GetSignalPins(xxx, True)",	"",									"",										"yyy.Name\Pin Type:PinTypeShort(yyy)\Pin Signal:yyy.Net\Sheet:PinSheet(yyy)",								"List of only invisible pins not used in gates but connected to a net (with optional type and signal)"), _
		Array("Used Gates", 				"Used Part Gates:", 			5,	Simple,	"aGate","xxx.Gates",					"xxx.Gates",				"GetPartGatesBySheet(xxx, cont)",	"GetPartGatesBySheet(xxx, cont)",		"yyy.Name\Sheet:yyy.Sheet",																					"List of installed (visible) part gates with gate location info"), _
		Array("Unused Gates", 				"Unused Gates:", 				5,	Simple,	"aGate","xxx.UnusedGates",				"xxx.UnusedGates",			"",	 								"",										"yyy.Name",																									"List of uninstalled part gates"), _
		Array("Sheets", 					"Sheets:", 						5,	Simple,	"sht",	"xxx.Sheets",					"xxx.Sheets",				"",									"",										"yyy.Name",																									"List of sheets where the part gates are located"), _
		Array("Attributes", 				"Part Attributes:", 			1,	Attr,	"attr",	"xxx.Attributes",				"xxx.Attributes",			"xxx.Attributes",					"xxx.Attributes",						"yyy.Name",																									"List of part attributes")  _
	), _
	Array( _
		Array("Gate Pins",					"Gate Pins:", 			 		5,	Simple,	"aPin",	"xxx.Pins",						"GetSortedPins(xxx)",		"xxx.Pins",							"GetSortedPins(xxx)",					"yyy.Name\Pin Type:PinTypeShort(yyy)\Pin Signal:ObjName(yyy.Net)",											"List of all gate pins (with optional type, location, and signal info)"), _
		Array("Connected Pins",				"Connected Gate Pins:", 		5,	Simple,	"aPin",	"GetConPins(xxx)",				"GetConPins(xxx, True)",	"GetConPins(xxx)",					"GetConPins(xxx, True)",				"yyy.Name\Pin Type:PinTypeShort(yyy)\Pin Signal:yyy.Net",													"List of only gate pins connected to a net (with optional type, location, and signal info"), _
		Array("Unconnected Pins",			"Unconnected Gate Pins:", 		5,	Simple,	"aPin",	"GetUnconPins(xxx)",			"GetUnconPins(xxx, True)",	"GetUnconPins(xxx)", 				"GetUnconPins(xxx, True)",				"yyy.Name\Pin Type:PinTypeShort(yyy)",																		"List of only Gate pins not connected to a net with pin type and location info") _
	), _
	Array( _
		Array("Net Pins",					"Net Pins:", 				 	5,	Simple,	"aPin",	"xxx.Pins",						"GetSortedPins(xxx)",		"GetPinsBySheet(xxx, cont)",		"GetPinsBySheet(xxx, cont, True)",		"yyy.Name\Pin Type:PinTypeShort(yyy)\Gate:PinGate(yyy)\Sheet:PinSheet(yyy)",								"List of all pins connected to the net (with optional type, gate, and location info)"), _
		Array("Signal Pins",				"Signal Net Pins:", 			5,	Simple,	"aPin",	"GetSignalNetPins(xxx)",		"GetSignalNetPins(xxx, True)","",								"",										"yyy.Name\Pin Type:PinTypeShort(yyy)\Sheet:PinSheet(yyy)",													"List of signal net pins invisible in the schematic (with optional type and location info)"), _
		Array("Net Gates", 					"Net Gates:", 					5,	Simple,	"aGate","GetNetGates(xxx)",				"GetNetGates(xxx, True)",	"GetNetGatesBySheet(xxx, cont)",	"GetNetGatesBySheet(xxx, cont, True)",	"yyy.Name\Sheet:yyy.Sheet",																					"List of gates the net goes through (with optional location info)"), _
		Array("Sheets", 					"Sheets:", 						5,	Simple,	"sht",	"xxx.Sheets",					"xxx.Sheets",				"",									"",										"yyy.Name",																									"List of sheets where the net pins are located"), _
		Array("Attributes", 				"Net Attributes:", 				1,	Attr,	"attr",	"xxx.Attributes",				"xxx.Attributes",			"xxx.Attributes",					"xxx.Attributes",						"yyy.Name",																									"List of net attributes") _
	), _
	Array( _
	), _
	Array( _
		Array("Parts", 						"Parts:", 						5,	Simple,	"part",	"xxx.Components",				"",							"GetPartsByPartType(cont, xxx)",	"",										"yyy.Name\Value:AttrVal(yyy, ""Value"")\Tolerance:AttrVal(yyy, ""Tolerance"")",								"List of parts of this part type"), _
		Array("Unused Gates", 				"Unused Gates:", 				5,	Simple,	"aGate","GetUnusedGatesByPartType(cont, xxx)",	"",					"GetUnusedGatesByPartType(cont, xxx)",	"",									"yyy.Name",																									"List of unused gates by part type"), _
		Array("Unused Pins",	 			"Unused Part Pins:", 			5,	Simple,	"aPin",	"GetUnusedPinsByPartType(cont, xxx)",	"",					"GetUnusedPinsByPartType(cont, xxx)",	"",									"yyy.Name",																									"List of unused pins  by part type") _
	)  _
)

'Array of lists of primary ojects
'	List Name ID,			Hdr,NPL,OutType,VarName,Collection,		  SortColl,	Rsv2,Out																																ObjectType
Const primList = Array( _
	Array( _
		Array("Parts",		"", 1,	Simple,	"part",	"cont.Components",		"",	,,	"xxx.Name\Part Type:xxx.PartType\PCBDecal:xxx.PCBDecal\Value:AttrVal(xxx, ""Value"")\Tolerance:AttrVal(xxx, ""Tolerance"")",	"") _
	), _
	Array( _
		Array("Gates",		"", 1,	Simple,	"aGate","cont.Gates",			"",	,,	"xxx.Name",																														"") _
	), _
	Array( _
		Array("Nets",		"", 1,	Simple,	"aNet",	"cont.Nets",			"",	,,	"xxx.Name\Net Width:xxx.Width",																									"") _
	), _
	Array( _
		Array("Pins", 		"", 1,	Simple,	"aPin",	"GetSortedPins(cont)",	"",	,,	"xxx.Name\Pin Type:PinTypeShort(xxx)\Pin Signal:ObjName(xxx.Net)\Gate:PinGate(xxx)\Sheet:PinSheet(xxx)",						"") _
	), _
	Array( _
		Array("Part Types", "", 1,	Simple,	"pkg",	"cont.PartTypes",		"",	,,	"xxx.Name",																													"") _
	) _
)
...
Const objProps = Array("Attributes, Part Type, PCB Decal, Logic Family, Pin Count.", _
						"Number, SwapClass, Position, etc.", _
						"Pin Count, Width.", _
						"Signal, ElectricalType, SwapClass, Position, etc.", _
						"Logic Family, Pin Count, Unused Pin Count")
						
Const objLists = Array("For example: Used Gates, Unused Gates, Unused Pins, Attributes, etc.", _
						"For example: Connected Pins, Unconnected Pins.", _
						"For example: Net Pins, Connected Gates.", _
						"", _
						"For example: List of parts of this type")
Const attrW = 32
Const contHeader = "Sheet"
...
Sub Page1
	Begin Dialog UserDialog 560,280,"Introduction",.dlgfunc ' %GRID:10,7,1,1
		Text 160,7,220,14,"Welcome to the Basic Script Wizard.",.Text2
		Text 10,28,540,42,"This program is completely written in PADS Logic's Basic script language. It will generate a skeleton Basic script for extracting database information in different application formats including RTF, Excel, and HTML.",.Text6
		...
		Text 10,119,540,28,"After completing this Wizard, you can extend the resulting Basic script by manually writing additional code.",.Text7
		GroupBox 10,189,540,42,"Basic Script Destination Directory",.Group1
...
Sub Page2
	Begin Dialog UserDialog 560,280,"Format",.dlgfunc ' %GRID:10,7,1,1
		Text 40,14,390,28,"Choose the target application format for the new Basic script. Resulting Basic code will vary depending on your choice.",.Text1
		OptionGroup .Target
...
Sub Page3
	Begin Dialog UserDialog 560,280,"Report Type",.dlgfunc ' %GRID:10,7,1,1
		Text 30,14,520,14,"Please choose which kind of report you want the new Basic script to generate:",.Text1
		OptionGroup .Container
			OptionButton 30,49,210,14,"Sche&matic-Based Reports",.Sch
			OptionButton 30,119,200,14,"&Sheet by Sheet Reports",.Sht
			OptionButton 30,189,210,14,"Reports for the &Given Sheet",.GivenCont
		Text 60,70,430,28,"Reports will contain database items collected from the entire design. Sheet information will not be taken into account.",.Text2
		Text 60,140,410,28,"Database items will be collected from each sheet individually. Sheets are sorted by number or by name.",.Text5
		PushButton 330,252,100,21,"&Next >",.NextBtn
		PushButton 230,252,100,21,"< &Back",.BackBtn
		CancelButton 450,252,100,21
		Text 10,231,540,14,"___________________________________________________________________",.Line
	...
	While True
		...
		End If
		curDlg = "3"
		n = ActiveDocument.Sheets.Count
		ReDim ContArray(n)
		ContArray(0) = ACTIVE_SHEET
		For i = 1 To n
			ContArray(i) = ActiveDocument.Sheets(i)
		Next
		dlg.ContList = IIf(givenCont = "", ACTIVE_SHEET, givenCont)
		n =  Dialog(dlg, 1)
		If n = 0 Then Abort
		...
		Else 
			contObj = IIf(givenCont = ACTIVE_SHEET, "ActiveDocument.ActiveSheet", "ActiveDocument.Sheets(" & Chr(34) & givenCont & Chr(34) & ")")
		End If
...
Sub Page4
	Begin Dialog UserDialog 560,280,"Database Object",.dlgfunc ' %GRID:10,7,1,1
		...
		OptionGroup .primObj
			OptionButton 50,56,60,14,"&Parts",.itPart
			OptionButton 140,56,70,14,"&Gates",.itGate
			OptionButton 240,56,60,14,"&Nets",.itNet
			OptionButton 330,56,90,14,"P&ins",.itPin
			OptionButton 430,56,100,14,"Part &Types",.itPartType
		CheckBox 50,119,480,14,"Calculate the total number of primary objects in the design",.primTotal
		CheckBox 50,168,490,14,"Output installed objects only.",.installedOnly
		CheckBox 50,168,460,14,"Calculate the number of primary objects for each sheet individually",.primContTotal
		PushButton 330,252,100,21,"&Next >",.NextBtn
		PushButton 230,252,100,21,"< &Back",.BackBtn
		CancelButton 450,252,100,21
		Text 10,231,540,14,"___________________________________________________________________",.Line
...
Sub Page5
	Begin Dialog UserDialog 560,280,"Data Type",.dlgfunc ' %GRID:10,7,1,1
		Text 20,14,530,35,"You have chosen XXX as the primary type of database object for the Basic report. Now you can choose what data the new Basic script will extract from a XXX.",.Text1
...
Sub Page6B
	Begin Dialog UserDialog 560,280,"Associated Objects",.dlgfunc ' %GRID:10,7,1,1
		...
		Text 10,21,540,14,"Basic Script will generate selected list(s) for each XXX entry in the report.",.Text12
...
Sub Page7
	Begin Dialog UserDialog 560,280,"Report Options",.dlgfunc ' %GRID:10,7,1,1
		Text 10,7,540,14,"Select additional report options for the Basic script.",.Text1
...
Sub Page8
	Begin Dialog UserDialog 560,280,"Output Files",.dlgfunc ' %GRID:10,7,1,1
		Text 10,7,550,14,"Select the name of the report file to which the new Basic Script will extract information.",.Text1
		...
		Text 10,231,540,14,"___________________________________________________________________",.Line
		Text 10,182,460,14,"&Enter the name of the resulting Basic script (without BAS extension):",.Text4
	...
	While True
		...
		If n = 2 Then
			...
			MacroRun scriptPath
		Else
			MsgBox "Basic Script generated successfully in the location:" & vbCr & scriptPath, vbInformation, AppName
		End If
...
Function EditCondition
	...
	Begin Dialog UserDialog 580,189,"Condition",.EditCondFunc ' %GRID:10,7,1,1
		...
		Text 10,119,540,28,"Notice that you can always modify resulting Basic code manually to assign conditions with unlimited level of complexity.",.Text1
	End Dialog
	Dim dlg As UserDialog
...
Function CheckMeasure(m) As Boolean
	...
	If m.Name = "Unknown" Or (m.unit(plogMeasureFormatLong) = "" And m.unit(plogMeasureFormatCurrent) = "") Then
...
Private Function dlgfunc(DlgItem$, Action%, SuppValue&) As Boolean
	...
	Case 1 ' Dialog box initialization
		...
		ElseIf curDlg = "4" Then
			i = DlgValue("primObj")
			DlgVisible "installedOnly", AppId = 2 And primData(i)(6) 'obj allowed for cont reports
			DlgVisible "primContTotal", primData(i)(6)
...
Sub 	GenTextCode
	Print #1, "'This script has been generated by "; AppName; "'s Basic Script Wizard on "; Now
	Print #1, "'It will create reports in "; formatName(target)
	Print #1, "'You can use the following code as a skeleton for your own Basic scripts"
	GenDeclarationCode
	Print #1
	Print #1, "Sub Main"
		nTabs = 1
	...
	If repByContainer Then 
		If primContTotal And (Not XLS Or Not Table) Then
			If FilterCondition = "" Then
				Print #1, Indent; OutBoldLine; Chr(34); primName; " count on the "; ContName(1); ": "; ListIndent; Chr(34); DLMT; "Subtotal"
			Else
				Print #1, Indent; OutBoldLine; Chr(34); "Number of "; primName; "s on the "; ContName(1); " matching requested conditions: "; Chr(34); DLMT; "Subtotal"
			End If
			Print #1, Indent; NewLine
		End If
	End If
...
Sub GenCodeForConnectedPins
	...
	Print #1,	"	Set GetConPins = ActiveDocument.GetObjects(0) 'create empty collectin
	Print #1,	"	For Each aPin In obj.Pins
...
Sub GenCodeForPartCountOfType
	...
	Print #1, "		If part.PartType = PartType Then PartCountOfType = PartCountOfType + 1
	Print #1, "	Next
	Print #1, "End Function
End Sub
...
'Declare Win32 functions and structures not supported in Basic laguage directly
Type CHOOSECOLOR
    lStructSize As Long 
    hwndOwner As Long 
    hInstance As Long
